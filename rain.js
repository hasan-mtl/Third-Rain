...
(function () {
  ...
  if (basinCanvas && seaCanvas && !reduce && window.matchMedia("(min-width: 901px)").matches) {
    ...
    var SEA_FRAG = [
      ...
      "  float yShift = shoreN * 2.0 * NOISE_AMPLITUDE;",
      "  float d0 = uRes.y * (1.0 - DISSOLVE_HEIGHT) + yShift;",
      "  float d1 = uRes.y - 4.0 + yShift;",
      "  float dissolve = smoothstep(d0, d1, px.y);",
      "  col = mix(col, vec3(0.98, 0.973, 0.961), dissolve);",
