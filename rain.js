...
(function () {
  ...
  if (basinCanvas && seaCanvas && !reduce && window.matchMedia("(min-width: 901px)").matches) {
    ...
    var SEA_FRAG = [
      ...
      "  float alpha = 0.92;",
      "",
      "  /* the shore: the last wave laps onto the page and slides back */",
      "  float lap = sin(uTime * 0.5) * 5.0 + sin(uTime * 0.27 + 1.7) * 3.0;",
      "  float edgeN = (vnoise(vec2(px.x * 0.012, uTime * 0.18)) - 0.5) * 22.0",
      "              + (vnoise(vec2(px.x * 0.05 + 7.0, uTime * 0.3)) - 0.5) * 8.0;",
      "  float shoreY = uRes.y - 46.0 + lap + edgeN;",
      "  float toShore = shoreY - px.y;",
      "  /* swash: the water thins and pales as it runs up the paper */",
      "  float swash = 1.0 - smoothstep(0.0, 34.0, toShore);",
      "  col = mix(col, vec3(0.94, 0.95, 0.95), swash * 0.42);",
      "  alpha *= 1.0 - swash * 0.25;",
      "  /* foam at the lap line: broken, patchy, alive */",
      "  float f1 = vnoise(vec2(px.x * 0.05, px.y * 0.05 - uTime * 0.8));",
      "  float f2 = vnoise(vec2(px.x * 0.16 + 31.0, uTime * 0.55));",
      "  float band = exp(-abs(px.y - shoreY) * 0.14);",
      "  float foam = band * smoothstep(0.34, 0.78, f1 * 0.6 + f2 * 0.55);",
      "  float ftw = step(0.93, hash21(floor(vec2(px.x * 0.5, px.y * 0.5 + floor(uTime * 3.0)) / 2.0)));",
      "  foam = clamp(foam + ftw * band * 0.35, 0.0, 1.0);",
      "  col = mix(col, vec3(0.975, 0.995, 1.0), foam * 0.85);",
      "  /* under the line: backwash wisps sliding home, then clean paper */",
      "  float below = clamp((px.y - shoreY) / 26.0, 0.0, 1.0);",
      "  float wisp = smoothstep(0.58, 0.95, vnoise(vec2(px.x * 0.035, px.y * 0.09 - uTime * 0.4))) * (1.0 - below);",
      "  alpha *= 1.0 - smoothstep(0.0, 20.0, px.y - shoreY);",
      "  alpha = max(alpha, clamp(foam * 0.85 + wisp * 0.3, 0.0, 1.0) * (1.0 - below * below));",
      "  col = mix(col, vec3(0.975, 0.995, 1.0), wisp * 0.5 * clamp((px.y - shoreY) * 0.2, 0.0, 1.0));",
      "",
