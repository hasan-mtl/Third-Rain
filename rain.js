...
(function () {
  ...
  /* ---------- contact header: the particle "rain." wordmark (mini hero crest) ---------- */
  (function () {
    var host = qs("[data-chat-mark]");
    if (!host) return;
    ...
    if (!ctx) return;
    var dpr = Math.min(window.devicePixelRatio || 1, 2);
    var W = 0, H = 0, parts = [], raf = null, t0 = performance.now(), last = t0, running = false, inited = false;

    function build() {
      var r = host.getBoundingClientRect();
      W = Math.max(40, Math.round(r.width));
      H = Math.max(18, Math.round(r.height));
      ...
      var octx = off.getContext("2d");
      var fontPx = Math.min(H * 0.9, W * 0.33);
      octx.font = "italic 600 " + Math.round(fontPx) + "px 'Cormorant Garamond', Georgia, serif";
      octx.textBaseline = "alphabetic";
      octx.fillStyle = "#fff";
      var met = octx.measureText("rain.");
      octx.fillText("rain.", (W - met.width) / 2, H * 0.5 + fontPx * 0.33);
      var data = octx.getImageData(0, 0, W, H).data;
      parts = [];
      for (var yy = 0; yy < H; yy += 2) {
        for (var xx = 0; xx < W; xx += 2) {
          if (data[(yy * W + xx) * 4 + 3] > 130) {
            parts.push({
              tx: xx, ty: yy,
              x: W * 0.5 + (Math.random() - 0.5) * W * 1.3,
              y: Math.random() * H,
              hue: Math.random(),
              ph: Math.random() * 6.283
            });
            if (parts.length >= 600) { yy = H; break; }
          }
        }
      }
    }

    function draw(time, settle) {
      ctx.clearRect(0, 0, W, H);
      for (var i = 0; i < parts.length; i++) {
        var p = parts[i];
        var a = 0.5 + 0.4 * Math.sin(time * 2.2 + p.ph * 2);
        if (a < 0.26) a = 0.26;
        ctx.fillStyle = p.hue < 0.5
          ? "rgba(158,218,255," + a.toFixed(3) + ")"
          : "rgba(128,240,218," + a.toFixed(3) + ")";
        var jx = settle ? Math.sin(time * 1.8 + p.ph) * 0.5 : 0;
        var jy = settle ? Math.cos(time * 1.5 + p.ph) * 0.5 : 0;
        ctx.fillRect(p.x + jx - 0.85, p.y + jy - 0.85, 1.7, 1.7);
      }
    }
    ...
    function play() { if (!running) { running = true; last = performance.now(); raf = requestAnimationFrame(step); } }
    ...
    function init() {
      ...
      if (reduce) {
        for (var i = 0; i < parts.length; i++) { parts[i].x = parts[i].tx; parts[i].y = parts[i].ty; }
        draw(0, false);
        return;
      }
      if ("IntersectionObserver" in window) {
      ...
      window.addEventListener("resize", function () {
        clearTimeout(rt);
        rt = setTimeout(function () { stop(); build(); play(); }, 220);
      });
    }
